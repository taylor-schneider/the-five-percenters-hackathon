...
MEASURED FACTS ONLY. This module reports what is true of a squad -- salaries,
averages, league percentiles, who occupies which slot. It does not decide what
is good or bad: `health_status`, `problem_score` and `headline` are left null
here and filled in by the Gap Finder agent's verdict (see
services/orchestration.py). Keeping the two apart is what stops a formula from
quietly impersonating an agent.
"""
...
def resolve_minimums(
    ...
    """Squad minimums per position group, request overrides on top of config."""
...
def to_insight_model(record: InsightRecord) -> AgentInsight:
    from ..models.enums import AgentName, InsightType, Severity
...
def roster_rows(
    ...
    for entry in sorted(entries, key=lambda e: (_slot_order(e.position), -e.value_score)):
        ...
        if include_benchmarks and ctx is not None:
            ...
            # health_status is deliberately left unset -- it is the agent's call.
        rows.append(row)
    return rows
...
def build_pitch_map(
    ...
    """Slot geometry and occupancy. Carries no judgement -- see module docstring."""
    ...
    for position in FORMATIONS.get(formation, FORMATIONS["4-3-3"]):
        ...
        slots.append(
            PitchSlot(
                ...
                x=x,
                y=y,
                # Judgement fields stay null here; the Gap Finder verdict fills them.
                health_status=None,
                problem_score=None,
