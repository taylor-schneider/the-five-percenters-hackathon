...
from ...data_models.roster_entry import RosterEntryRecord
...
class LeagueContext:
    ...
    def cost_p50(self, position: Position) -> int:
        costs = self.cost_by_position.get(position, [])
        return int(round(statistics.median(costs))) if costs else 0

    def value_p50(self, position: Position) -> float:
        values = self.value_by_position.get(position, [])
        return round(statistics.median(values), 1) if values else 0.0

    def sample_size(self, position: Position) -> int:
        """Below 5, the UI mutes the benchmark columns (api.md 3.7)."""
        return len(self.value_by_position.get(position, []))

    def positions(self) -> list[Position]:
        """Positions with at least one league entry."""
        return sorted(self.value_by_position, key=lambda p: p.value)
