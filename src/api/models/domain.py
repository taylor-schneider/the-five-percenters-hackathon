...
class PitchSlot(ApiModel):
    ...
    health_status: HealthStatus | None = Field(
        default=None,
        description="Agent judgement. null when no Gap Finder run has been applied.",
    )
    problem_score: float | None = Field(default=None, ge=0, le=1)
    ...
    headline: str | None = Field(
        default=None,
        description="Agent's one-line tooltip. Present only when health_status is red.",
    )
